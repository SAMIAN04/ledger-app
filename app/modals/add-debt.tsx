// app/modals/add-debt.tsx — GlassModal style
...
export default function AddDebtModal() {
  ...
  async function submit() {
    if (!lenderName.trim() || !amount) { Alert.alert('Missing fields', 'Lender name and amount are required.'); return; }
    ...
    try {
      ...
      const debtId = await fbAdd('debts', { lenderName: lenderName.trim(), amount: amt, walletId, dueDate, status: 'pending', notes, paymentHistory: [], profilePic: profilePic || null, createdAt: now });
      if (wallet) await fbUpdate('wallets', walletId, { balance: (wallet.balance || 0) + amt });
      await fbAdd('transactions', { type: 'debt', title: `Debt from ${lenderName.trim()}`, amount: amt, walletId, date: now.slice(0, 10), createdAt: now, notes, personName: lenderName.trim(), profilePic: profilePic || null, linkedRecordId: debtId });
      closeModal();
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <GlassModal accentColor="#f97316" secondaryColor="#c2410c">
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          <Animated.View style={[{ flex: 1 }, { opacity, transform: [{ translateY: slideY }] }]}>
            ...
            <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="always">
              <View style={styles.infoBanner}><Text style={styles.infoBannerText}>⚠️ Recording debt adds the borrowed amount to your wallet balance.</Text></View>
              <View style={styles.pfpSection}>
                ...
                <Text style={styles.pfpHint}>Add lender photo (optional)</Text>
              </View>
              <InputField label="Lender / Source" value={lenderName} onChangeText={setLenderName} placeholder="Bank name or person" />
              <InputField label="Amount" value={amount} onChangeText={setAmount} keyboardType="decimal-pad" placeholder="0.00" />
              <SelectField label="To Wallet" value={walletId} options={walletOptions} onChange={setWalletId} />
              <DatePickerField label="Due Date (optional)" value={dueDate} onChange={setDueDate} placeholder="Select due date" disablePast />
              <InputField label="Notes (optional)" value={notes} onChangeText={setNotes} placeholder="Purpose of debt..." />
              <View style={styles.footer}>
                ...
                <GradientButton label="Record Debt" onPress={submit} loading={loading} colors={['#f97316', '#c2410c']} style={styles.submitBtn} />
              </View>
            </ScrollView>
          </Animated.View>
...
const styles = StyleSheet.create({
  ...
  headerIcon:    { width: 38, height: 38, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(249,115,22,0.22)' },
  ...
  infoBanner:    { backgroundColor: 'rgba(249,115,22,0.14)', borderRadius: 12, padding: 12, marginBottom: 20, ...SHADOW.raised },
  infoBannerText:{ color: '#fdba74', fontSize: 12.5, lineHeight: 18 },
  pfpSection:    { alignItems: 'center', marginBottom: 24 },
  pfpBtn:        { position: 'relative', marginBottom: 8 },
  pfpImage:      { width: 80, height: 80, borderRadius: 40, borderWidth: 2, borderColor: 'rgba(249,115,22,0.5)' },
  ...
  pfpBadge:      { position: 'absolute', bottom: 0, right: 0, width: 24, height: 24, borderRadius: 12, backgroundColor: '#f97316', alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: COLORS.sheet },
